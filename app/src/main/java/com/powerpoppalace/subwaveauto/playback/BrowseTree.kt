...
class BrowseTree(var api: StationApi) : MediaLibrarySession.Callback {
    ...
    internal fun resolveMediaItems(requested: List<MediaItem>): MutableList<MediaItem> {
        ...
        // Controllers (AA) send bare mediaIds with no URI — every requested item
        // maps to the fully-formed live item with a fresh cache-busted stream URI.
        return requested.map { liveMediaItem(api) }.toMutableList()
    }

    /** Root children: the single live item. */
    internal fun rootChildren(): List<MediaItem> = listOf(browseLiveItem())
